...
# ZeroCut Gaming AI Run 112 - Whisper UX candidate.
# P1 scope: truthful local whisper.cpp transcription exposed as a user-reviewable product gate.
...
def _enhance_whisper_ui_assets() -> None:
    """Add a truthful, reviewable local-transcript card without rebuilding the approved UI."""
    marker = "zerocut-whisper-run112"
    html_path, js_path, css_path = WEB / "index.html", WEB / "app.js", WEB / "styles.css"
    try:
        html = html_path.read_text(encoding="utf-8")
        if marker not in html:
            card = """
<section id="whisperCard" class="panel zc-whisper-card" data-feature="zerocut-whisper-run112" aria-labelledby="whisperTitle">
  <div class="zc-whisper-head">
    <div>
      <div class="eyebrow">AI locale · Whisper</div>
      <h2 id="whisperTitle">Trascrizione audio</h2>
      <p class="muted">Trascrive il video sul PC. Nessun cloud obbligatorio e nessun successo se il testo non è realmente utilizzabile.</p>
    </div>
    <span id="whisperHealth" class="zc-whisper-badge" data-state="idle">Non eseguita</span>
  </div>
  <div class="zc-whisper-actions">
    <button id="runWhisperTranscript" type="button" class="primary">Trascrivi audio</button>
    <span id="whisperMeta" class="muted">Carica un video e avvia la trascrizione.</span>
  </div>
  <pre id="whisperPreview" class="zc-whisper-preview" aria-live="polite">L'anteprima comparirà qui.</pre>
</section>
"""
            if "</main>" in html:
        ...
        if marker not in js:
            js += r'''
// zerocut-whisper-run112
(() => {
  const btn = document.getElementById("runWhisperTranscript");
  const badge = document.getElementById("whisperHealth");
  const meta = document.getElementById("whisperMeta");
  const preview = document.getElementById("whisperPreview");
  if (!btn || !badge || !meta || !preview) return;

  const render = (t) => {
    if (!t) {
      badge.dataset.state = "idle"; badge.textContent = "Non eseguita";
      meta.textContent = "Carica un video e avvia la trascrizione.";
      preview.textContent = "L'anteprima comparirà qui.";
      return;
    }
    badge.dataset.state = t.usable ? "ok" : "error";
    badge.textContent = t.usable ? "Pronta" : "Non utilizzabile";
    meta.textContent = t.usable
      ? String(t.segment_count) + " segmenti · " + String(t.word_count) + " parole" + (t.cache_hit ? " · cache" : "")
      : "Trascrizione non disponibile: " + String(t.reason || "errore sconosciuto");
    preview.textContent = t.text_preview || "Nessun testo utilizzabile.";
  };

  async function loadLatest() {
    try {
      const res = await fetch("/api/transcript/latest", {cache:"no-store"});
      if (res.ok) render((await res.json()).transcript);
    } catch (_) {}
  }

  btn.addEventListener("click", async () => {
    btn.disabled = true;
    badge.dataset.state = "working"; badge.textContent = "Trascrizione…";
    meta.textContent = "Analisi locale in corso. Il video non viene inviato a servizi cloud.";
    try {
      ...
      render(data.transcript || {usable:false, reason:data.error || ("HTTP_" + String(res.status))});
    } catch (err) {
      render({usable:false, reason:String(err)});
    } finally {
      btn.disabled = false;
    }
  });
...
'''
        ...
        if marker not in css:
            css += r'''
/* zerocut-whisper-run112 */
.zc-whisper-card{margin-top:18px;display:grid;gap:14px}
.zc-whisper-head{display:flex;gap:18px;justify-content:space-between;align-items:flex-start}
.zc-whisper-head h2{margin:.15rem 0 .35rem}
.zc-whisper-actions{display:flex;gap:12px;align-items:center;flex-wrap:wrap}
.zc-whisper-badge{border:1px solid rgba(255,255,255,.16);border-radius:999px;padding:7px 11px;font-size:.82rem;font-weight:700;white-space:nowrap}
.zc-whisper-badge[data-state="ok"]{border-color:rgba(74,222,128,.5)}
.zc-whisper-badge[data-state="error"]{border-color:rgba(248,113,113,.55)}
.zc-whisper-badge[data-state="working"]{border-color:rgba(250,204,21,.5)}
.zc-whisper-preview{margin:0;min-height:84px;max-height:220px;overflow:auto;white-space:pre-wrap;word-break:break-word;padding:14px;border-radius:12px;background:rgba(0,0,0,.18);border:1px solid rgba(255,255,255,.08);font:inherit;line-height:1.55}
@media(max-width:700px){.zc-whisper-head{display:grid}.zc-whisper-badge{justify-self:start}.zc-whisper-actions .primary{width:100%}}
'''
...
def whisper_runtime_certification(audio_path: Path, *, timeout: float = 120.0) -> dict:
    """Certify Whisper only after a real semantic inference and fingerprint the exact runtime assets."""
    cap=discover_whisper()
    if not cap.get("available"):
        return {"certified":False,"executed":False,"reason":cap.get("reason_code","NOT_AVAILABLE"),
                "binary_fingerprint":None,"model_fingerprint":None}
    test=whisper_inference_selftest(Path(audio_path),timeout=timeout)
    binary_fp=_file_fingerprint(cap.get("binary"))
    model_fp=_file_fingerprint(cap.get("model"))
    certified=bool(test.get("executed") and test.get("passed") and binary_fp and model_fp
                   and binary_fp.get("sha256") and model_fp.get("sha256"))
    return {"certified":certified,"executed":bool(test.get("executed")),
            "reason":"OK" if certified else test.get("reason","CERTIFICATION_FAILED"),
            "selftest":test,"binary_fingerprint":binary_fp,"model_fingerprint":model_fp,
            "binary":cap.get("binary"),"model":cap.get("model")}
...
def transcript_user_summary(result: dict | None) -> dict:
    """Return bounded, truthful transcript information safe to persist/render in the UI."""
    ...
    for seg in segments:
        ...
        if end <= start:
            continue
        clean_segments.append({"start": round(start, 3), "end": round(end, 3), "text": text[:500]})
        words += len(text.split())
    usable = bool(row.get("ok") and clean_segments and words > 0)
    joined = " ".join(seg["text"] for seg in clean_segments)
    return {
        "usable": usable,
        "generated": bool(row.get("executed")),
        "reason": "OK" if usable else str(row.get("reason") or "EMPTY_TRANSCRIPT"),
        "language": str(row.get("language") or "auto"),
        "cache_hit": bool(row.get("cache_hit")),
        "segment_count": len(clean_segments),
        "word_count": words,
        "text_preview": joined[:1200],
        "source_sha256": str(row.get("source_sha256") or "")[:64],
        "model_sha256": str(row.get("model_sha256") or "")[:64],
    }
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            json_response(self, {
                ...
                "runtime": "Run119_WindowsReadiness",
                "pid": os.getpid(),
                "ffmpeg": _active_media_tool("ffmpeg"),
                "ffprobe": _active_media_tool("ffprobe"),
            })
            return
        if path == "/api/status":
            state = load_state()
            ...
            return
        if path == "/api/transcript/latest":
            state = load_state()
            json_response(self, {"ok": True, "transcript": state.get("last_transcript")})
            return
    ...
    def do_POST(self):
        ...
        try:
            if path == "/api/analyze/transcript":
                ...
                language = str(payload.get("language") or "auto").strip().lower()[:16] or "auto"
                result = transcribe_media_whisper(source, language=language, timeout=1800.0)
                summary = transcript_user_summary(result)
                state["last_transcript"] = summary
                save_state(state, expected_revision=analysis_base_revision)
                status = 200 if summary.get("usable") else 422
                body = {"ok": bool(summary.get("usable")), "transcript": summary}
                if summary.get("usable"):
                    body["segments"] = result.get("segments") or []
                else:
                    body["error"] = summary.get("reason") or "TRANSCRIPT_NOT_USABLE"
                json_response(self, body, status)
                return
...
def main():
    ...
    parser.add_argument("--ready-file", default=None, help="Scrive un handshake JSON atomico quando il server locale e pronto")
    ...
    ready_path = Path(args.ready_file).expanduser().resolve() if args.ready_file else None
    if ready_path:
        ...
        ready_tmp = ready_path.with_suffix(ready_path.suffix + ".tmp")
